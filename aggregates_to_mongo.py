...
def insert_aggregates_to_mongo():
    ...
    games = (spark.read.format("csv").option("header", True).load(f"gs://{bucket_name}/data/*.csv"))
    ...
    aggs = games.rdd.map(lambda x: add_csv_data_to_rdd(x)).collect()
    try:
